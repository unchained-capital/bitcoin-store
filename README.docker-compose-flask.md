...
**This app is using Flask 2.0.1 and Python 3.9.2**. The screenshot doesn't get
...
- **Flask defaults that are changed**:
    ...
    - `ProxyFix` middleware is enabled (check `bitcoinstore/app.py`)
...
```sh
git clone https://github.com/nickjj/docker-flask-example bitcoinstore
cd bitcoinstore
...
The app is named `bitcoinstore` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "bitcoinstore" and update a few Docker related resources.
