...
from bitcoinstore.model.product import FungibleProduct, NonFungibleProduct, NonFungibleSku
...
nfp_res = {
	...
	'userId':'steve',
	# expires tomorrow
	'expiration':(datetime.datetime.today() + datetime.timedelta(days=1)).replace(microsecond=0),
}
...
class TestReservations(ViewTestMixin):
	...
	def validateFields(self, json, reservation, expired):
		...
		assert parse(json['expiration']).timestamp() == reservation['expiration'].timestamp()
	...
	def test_create_fp_res(self):
		response = self.client.post(url_for("reservations.createFungible"), json=fp_res)
		assert 201 == response.status_code
		self.validateFields(response.get_json(), fp_res, expired=False)

		# check fungible product qty reserved was incremented
		response = self.client.get(url_for("products.queryFungible") + "?sku=" + fp['sku'])
		assert 200 == response.status_code
		assert fp_res['qty'] + fp['qty_reserved'] == response.json[0]['qty_reserved']

	def test_create_nfp_res(self):
		response = self.client.post(url_for("reservations.createNonFungible"), json=nfp_res)
		assert 201 == response.status_code
	...
	def test_create_fp_res_invalid(self):
		...
		response = self.client.post(url_for("reservations.createFungible"), json=fp_res_invalid)
		assert 400 == response.status_code
	...
	def test_create_nfp_res_invalid(self):
		...
		response = self.client.post(url_for("reservations.createNonFungible"), json=nfp_res_invalid)
		assert 400 == response.status_code
	...
	def test_query_fp_res(self):
		...
		response = self.client.post(url_for("reservations.createFungible"), json=fp_res2)
		assert 201 == response.status_code
		...
		response = self.client.get(url_for("reservations.queryFungible") + "?userId=" + fp_res["userId"])
		assert 200 == response.status_code
		assert 1 == len(response.get_json())
		assert fp_res['userId'] == response.json[0]['userId']

		response = self.client.get(url_for("reservations.queryFungible") + "?sku=" + fp_res["sku"])
		assert 200 == response.status_code
		assert 1 == len(response.get_json())
	...
	def test_get_nfp_res(self):
		...
		response = self.client.get(url_for("reservations.getNonFungible", id=id))
		assert 200 == response.status_code
		self.validateFields(response.get_json(), nfp_res, expired=False)
	...
	def test_query_nfp_res(self):
		...
		response = self.client.post(url_for("reservations.createNonFungible"), json=nfp_res2)
		assert 201 == response.status_code
		...
		response = self.client.get(url_for("reservations.queryNonFungible") + "?userId=" + nfp_res["userId"])
		assert 200 == response.status_code
		assert 1 == len(response.get_json())
		assert nfp_res['userId'] == response.json[0]['userId']

		response = self.client.get(url_for("reservations.queryNonFungible") + "?serial=" + nfp_res["serial"])
		assert 200 == response.status_code
		assert 1 == len(response.get_json())
	...
	def test_expire_nfp_res(self):
		...
		response = self.client.delete(url_for("reservations.expireNonFungible", id=id))
		assert 200 == response.status_code
	...
	def test_expire_fp_res(self):
		...
		response = self.client.delete(url_for("reservations.expireFungible", id=id))
		assert 200 == response.status_code
